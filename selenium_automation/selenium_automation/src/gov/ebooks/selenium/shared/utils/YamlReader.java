...
public class YamlReader {
    ...
    public static String setYamlFilePath(String packageString) {
    	String[] fn = packageString.split("\\.");
    	String projectName = fn[3];
    	String tier = getProperty("Config.properties", "tier").trim();
        
        if (tier.equalsIgnoreCase("dev")) {
            yamlFilePath = projectName +"/gov/ebooks/selenium/"+projectName+"/resources/DEV_TestData.yml";
        } else if (tier.equalsIgnoreCase("qa")) {
            yamlFilePath = projectName +"/gov/ebooks/selenium/"+projectName+"/resources/QA_TestData.yml";
        } else if (tier.equalsIgnoreCase("stag") || tier.equalsIgnoreCase("staging")) {
            yamlFilePath = projectName +"/gov/ebooks/selenium/"+projectName+"/resources/STAGING_TestData.yml";
        }  else if (tier.equalsIgnoreCase("prod") || tier.equalsIgnoreCase("production")) {
            yamlFilePath = projectName +"/gov/ebooks/selenium/"+projectName+"/resources/PROD_TestData.yml";
        } else {
